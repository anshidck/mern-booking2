    ...
    <>
      <div className="w-full flex gap-2 justify-center">
        <input value={photoLink}
               className="w-[50%] md:w-[50%] p-3 rounded-sm border outline-teal-700"
               onChange={ev => setPhotoLink(ev.target.value)}
               type="text" placeholder={'Add using a link ....jpg'}/>
        <button onClick={addPhotoByLink} className="bg-gray-200 px-4 rounded-2xl ">Add&nbsp;photo</button>
      </div>
      <div className="w-full mt-2 grid px-24 items-center gap-2 grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {addedPhotos.length > 0 && addedPhotos.map(link => (
          <div className="h-32 flex justify-center relative" key={link}>
